...
from networks.unet import Rec_Decoder
...
from dataset.covid import Covid_Multi, Covid
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='fundus', choices=['fundus', 'prostate', 'fundus_p', 'covid'], help='training dataset')
...
def test_fundus(encoder, seg_decoder, epoch, data_dir, datasetTest, output_path, batch_size=8, dataset='fundus'):
    ...
    with torch.no_grad():
        for batch_idx, (data, target, target_orgin, ids) in enumerate(tbar):
            ...
            ### iw
            x, _ = encoder(data)
            prediction = torch.sigmoid(seg_decoder(x))
...
def test_covid(encoder, seg_decoder, epoch, data_dir, datasetTest, output_path, batch_size=8, dataset='fundus'):
    encoder.eval()
    seg_decoder.eval()
    data_dir = os.path.join(data_dir, dataset)
    transform = Compose([trans.Resize((256, 256)), trans.Normalize()])
    testset = Covid(base_dir=data_dir, split='test',
                     domain_idx=datasetTest, transform=transform)
    ...
    with torch.no_grad():
        for batch_idx, (data, target, target_orgin, ids) in enumerate(tbar):
            ...
            prediction = torch.sigmoid(seg_decoder(x))
            # prediction = torch.sigmoid(seg_decoder(encoder(data)))
            prediction = torch.nn.functional.interpolate(prediction, size=(target_orgin.size()[-2], target_orgin.size()[-1]), mode="bilinear")
            data = torch.nn.functional.interpolate(data, size=(target_orgin.size()[-2], target_orgin.size()[-1]), mode="bilinear")

        #     for i in range(target_orgin.size()[0]):
        #         pred_cv = np.asarray(prediction[i].cpu()[1]) * 255
        #         mask_cv = np.asarray(target_orgin[i]) * 255
        #         # cv2.imshow('pred', pred_cv)
        #         # cv2.imshow('mask', mask_cv)
        #         # cv2.waitKey(0)
        #     #exit()
        #         dice_coeff = binary.dc(np.asarray(prediction[i].cpu(), dtype=np.bool_),
        #                     np.asarray(target_orgin[i], dtype=np.bool_))
        #         val_dice += dice_coeff
        #         total_num += 1
        # val_dice /= total_num
        # # print(total_num)
...
def test_prostate(encoder, seg_decoder, epoch, data_dir, datasetTest, output_path, batch_size=8, dataset='prostate'):
    ...
    for file_name in tbar:
        ...
        for ii in range(int(np.floor(image.shape[0] // batch_size))):
            ...
            ##### iw
            img_feats, _ = encoder(vol)
            pred = torch.softmax(seg_decoder(img_feats), dim=1)
...
def train_fundus(trainloader_list, encoder, seg_decoder, rec_decoder, writer, args, optimizer, dataloader_length_max, domain_idx_list, batch_size_list):
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample_batches in enumerate(tbar):
            ...
            img_feats, iw_loss = encoder(img_multi)
            pred_soft_1 = torch.sigmoid(seg_decoder(img_feats))
            pred_soft_1 = torch.nn.functional.interpolate(pred_soft_1, size=mask_multi.shape[-2:], mode="bilinear", align_corners=False)
            ...
            loss += 1e-4 * (iw_loss[0] + iw_loss[1]) / 2
        ...
        if (epoch + 1) % 1 == 0:
            ...
            if (epoch + 1) % 30 == 0:
                torch.save(checkpoint, os.path.join(args.save_path, 'model_epoch{}_{}.pth'.format(epoch+1, avg_dice)))
...
def train_covid(trainloader_list, encoder, seg_decoder, rec_decoder, writer, args, optimizer, dataloader_length_max, domain_idx_list, batch_size_list):
    if args.consistency_type == 'mse':
        consistency_criterion = MSELoss()
    elif args.consistency_type == 'kd':
        consistency_criterion = KD
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 1 == 0:
            print("Test on target domain {}".format(args.test_domain_idx))
            with torch.no_grad():
                avg_dice = test_covid(encoder, seg_decoder, epoch, args.data_root, args.test_domain_idx, args.save_path, args.test_batch_size, dataset=args.dataset)
            if (epoch + 1) % 30 == 0:
                torch.save(checkpoint, os.path.join(args.save_path, 'model_epoch{}_{}.pth'.format(epoch+1, avg_dice)))
            if avg_dice >= previous_best:
                if previous_best != 0:
...
def train_prostate(trainloader_list, encoder, seg_decoder, rec_decoder, writer, args, optimizer, dataloader_length_max, domain_idx_list, batch_size_list):
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample_batches in enumerate(tbar):
            ...
            pred_1 = torch.nn.functional.interpolate(pred_1, size=mask_multi.shape[-2:], mode="bilinear", align_corners=False)
            pred_soft_1 = torch.nn.functional.interpolate(pred_soft_1, size=mask_multi.shape[-2:], mode="bilinear", align_corners=False)
        ...
        if (epoch + 1) % 1 == 0:
            ...
            if (epoch + 1) % 30 == 0:
                torch.save(checkpoint, os.path.join(args.save_path, 'model_epoch{}_{:.2f}.pth'.format(epoch+1, avg_dice)))
            if avg_dice >= previous_best:
                if previous_best != 0:
                    model_path = os.path.join(args.save_path, 'model_%.2f.pth' % (previous_best))
                    if os.path.exists(model_path):
...
def main(args):
    data_root = args.data_root + '/' + args.dataset
    ...
    dataset_zoo = {'fundus': Fundus_Multi, 'prostate': Prostate_Multi, 'fundus_p': Fundus_Multi, 'covid': Covid_Multi}
    transform = {'fundus': Compose([trans.Resize((256, 256)), trans.RandomScaleCrop((256, 256))]),
                 'prostate': None,
                 'fundus_p': Compose([trans.Resize((256, 256)), trans.RandomScaleCrop((256, 256))]),
                 'covid': Compose([trans.Resize((256, 256)), trans.RandomScaleCrop((256, 256))])}
    batch_size_list = {'fundus': fundus_batch_list[args.test_domain_idx] if args.test_domain_idx < 4 else None,
                       ...
                       'fundus_p': fundus_batch_list[args.test_domain_idx] if args.test_domain_idx < 4 else None,
                       'covid': fundus_batch_list[args.test_domain_idx] if args.test_domain_idx < 4 else None}
    ...
    for idx, i in enumerate(domain_idx_list):
        ...
        trainloader = DataLoader(trainset, batch_size=batch_size_list[args.dataset][idx], num_workers=1,
    ...
    if args.encoder == 'b0':
        encoder = Encoder_b0()
        state_dicts = torch.load('G:/MedDGseg/RAM_feed/code/pretrained/mit_b0.pth')
        # encoder.load_state_dict(state_dicts, strict=False)
        encoder.load_state_dict(state_dicts, strict=True)
        seg_decoder = Decoder_b0(num_classes=args.num_classes)
    elif args.encoder == 'b2':
        encoder = Encoder_b2()
        state_dicts = torch.load('G:/MedDGseg/RAM_feed/code/pretrained/mit_b2.pth')
        # encoder.load_state_dict(state_dicts, strict=False)
        encoder.load_state_dict(state_dicts, strict=True)
        seg_decoder = Decoder_b2(num_classes=args.num_classes)
    elif args.encoder == 'b3':
        encoder = Encoder_b3()
        state_dicts = torch.load('G:/MedDGseg/RAM_feed/code/pretrained/mit_b3.pth')
        # encoder.load_state_dict(state_dicts, strict=False)
        encoder.load_state_dict(state_dicts, strict=True)
        ...
        encoder = Encoder_b4()
        state_dicts = torch.load('G:/MedDGseg/RAM_feed/code/pretrained/mit_b4.pth')
        # encoder.load_state_dict(state_dicts, strict=False)
        encoder.load_state_dict(state_dicts, strict=True)
        seg_decoder = Decoder_b4(num_classes=args.num_classes)
    elif args.encoder == 'b5':
        encoder = Encoder_b5()
        state_dicts = torch.load('G:/MedDGseg/RAM_feed/code/pretrained/mit_b5.pth')
        # encoder.load_state_dict(state_dicts, strict=False)
        encoder.load_state_dict(state_dicts, strict=True)
    ...
    if args.rec:
        ...
        #     if args.encoder == 'b3':
        #         rec_decoder = Rec_Decoder(num_classes=args.num_classes)
        ...
        #                     lr=args.lr, betas=(0.9, 0.999))
        if args.encoder == 'b3':
                rec_decoder = RecDecoder_b3(num_classes=args.num_classes)
                optimizer = Adam([{"params": encoder.parameters(), 'lr': args.lr / 2},
                                {"params": seg_decoder.parameters(), 'lr': args.lr},
                                {"params": rec_decoder.parameters(), 'lr': args.lr}],
                                lr=args.lr, betas=(0.9, 0.999))
        if args.encoder == 'b4':
            rec_decoder = RecDecoder_b4(num_classes=args.num_classes)
            optimizer = Adam([{"params": encoder.parameters(), 'lr': args.lr / 2},
                            {"params": seg_decoder.parameters(), 'lr': args.lr},
                            {"params": rec_decoder.parameters(), 'lr': args.lr}],
                            lr=args.lr, betas=(0.9, 0.999))
        if args.encoder == 'b5':
            rec_decoder = RecDecoder_b5(num_classes=args.num_classes)
            optimizer = Adam([{"params": encoder.parameters(), 'lr': args.lr / 2},
                            {"params": seg_decoder.parameters(), 'lr': args.lr},
                            {"params": rec_decoder.parameters(), 'lr': args.lr}],
                            lr=args.lr, betas=(0.9, 0.999))
    ...
    if args.dataset == 'fundus':
        ...
        train_prostate(dataloader_list, encoder, seg_decoder, rec_decoder, writer,
                       args, optimizer, dataloader_length_max, domain_idx_list, batch_size_list[args.dataset])
    elif args.dataset == 'fundus_p':
        train_fundus(dataloader_list, encoder, seg_decoder, rec_decoder, writer,
                     args, optimizer, dataloader_length_max, domain_idx_list, batch_size_list[args.dataset])
    elif args.dataset == 'covid':
        train_covid(dataloader_list, encoder, seg_decoder, rec_decoder, writer,
                     args, optimizer, dataloader_length_max, domain_idx_list, batch_size_list[args.dataset])
    else:
...
if __name__ == '__main__':
    ...
    if args.epochs is None:
        args.epochs = {'fundus': 200, 'prostate': 200, 'fundus_p': 200, 'covid': 200}[args.dataset]
    if args.lr is None:
        args.lr = {'fundus': 5e-4, 'prostate': 3e-4, 'fundus_p': 5e-4, 'covid': 5e-4}[args.dataset]
    if args.num_classes is None:
        args.num_classes = {'fundus': 2, 'prostate': 2, 'fundus_p': 2, 'covid': 2}[args.dataset]
