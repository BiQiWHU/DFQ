...
class Covid(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(os.path.join(self.base_dir, cur_domain_name, id.split(' ')[0].replace('{}/'.format(cur_domain_name), '')))
        

        if self.split == 'test':
            mask = Image.open(os.path.join(self.base_dir, cur_domain_name, id.split(' ')[1].replace('{}/'.format(cur_domain_name), ''))).convert('L')
...
class Covid_Multi(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'test':
            ...
            # pre_mask = np.array(mask).astype(np.uint8)
            # mask = np.zeros((pre_mask.shape[0], pre_mask.shape[1], 2))
            # mask[pre_mask == 0] = [0, 1]
            # mask[pre_mask != 0] = [1, 1]
            # mask = mask.transpose(2, 0, 1)
            # mask = torch.from_numpy(np.array(mask)).float()
        ...
        if self.is_freq:
            ...
            other_img = Image.open(os.path.join(self.base_dir, other_domain_name, other_id.replace('{}/'.format(other_domain_name), ''))).resize((256, 256), Image.BILINEAR)
