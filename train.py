...
def test_covid(encoder, seg_decoder, epoch, data_dir, datasetTest, output_path, batch_size=8, dataset='fundus'):
    ...
    with torch.no_grad():
        ...
        #         # cv2.waitKey(0)
        #     #exit()
        #         dice_coeff = binary.dc(np.asarray(prediction[i].cpu(), dtype=np.bool__),
        #                     np.asarray(target_orgin[i], dtype=np.bool__))
        #         val_dice += dice_coeff
        #         total_num += 1
        # val_dice /= total_num
        # # print(total_num)
...
def test_prostate(encoder, seg_decoder, epoch, data_dir, datasetTest, output_path, batch_size=8, dataset='prostate'):
    ...
    for file_name in tbar:
        ...
        dice_coeff = binary.dc(np.asarray(processed_pred_y, dtype=np.bool__),
                            np.asarray(mask, dtype=np.bool__))
        val_dice += dice_coeff
        total_num += 1
